import os
import yaml
import mlflow
import models

from hashlib import sha256
from logging import Logger
from mlflow.models import infer_signature
from pandas import DataFrame, read_csv
from sklearn.metrics import roc_auc_score, accuracy_score, precision_score
from sklearn.metrics import recall_score, make_scorer
from sqlalchemy import create_engine

def create_experiment_id(config:dict, logger:Logger=None):
    ''' Creates an id from the experiment configuration and returns it. '''

    # Log config
    if (logger):
        logger.debug(f'create_experiment: config: {config}')

    # Create unique id
    h = sha256()

    keys = ['name', 'train_path', 'test_path']
    for k in keys:
        h.update(config[k].encode())
    
    h.update(config['model']['name'].encode())

    if ('preprocFunc' in config.keys()):
        h.update(config['preprocFunc'].encode())
    
    return h.hexdigest()[:64]

def load_data(data_path, logger:Logger=None):
    ''' Loads the data with the path, and returns X and y. '''

    # Log data_path
    if (logger):
        logger.debug(f'load_data: data_path: {data_path}')

    # Load data
    X = read_csv(data_path)
    y = X.pop('y')

    return X.to_numpy(), y.to_numpy()

def df_to_sql(df: DataFrame, db_url, table_name, logger:Logger=None):
    ''' Inserts the dataframe into the database. '''

    # Log db_url and table_name
    if (logger):
        logger.debug(f'df_to_sql: db_url: {db_url}')
        logger.debug(f'df_to_sql: table_name: {table_name}')

    # Connect to database
    engine = create_engine(db_url)
    with engine.connect() as con:
        df.to_sql(table_name, con, if_exists='append', index=False)
    
    return None

def get_model(config, logger:Logger=None):
    ''' Gets the model and returns it. '''

    # Log config
    if (logger):
        logger.debug(f'get_model: config: {config}')

    # Build model
    model = models.build_model(config, logger)

    return model

def evaluate(model, X, y):
    ''' Scores the binary classifier againts roc_auc, accuracy, precision
        and recall, and returns the results. '''
    
    # Define metrics
    metrics = {
        'roc_auc': roc_auc_score,
        'accuracy': accuracy_score,
        'precision': precision_score,
        'recall': recall_score
    }

    # Evaluate model against metrics
    results = {}
    for name, metric in metrics.items():
        results[name] = [make_scorer(metric)(model,X,y)]
    
    return results


def save_results(
        results,
        experiment_id,
        experiment_name,
        table_name,
        logger:Logger=None
    ):
    ''' Saves the results to the database and returns None. If the
        environment variable RESULTS_URL is none, then save_results returns
        none. '''
    
    # Get results url
    results_url = os.getenv('RESULTS_URL')
    
    # Log results url
    if (logger):
        logger.debug(f'save_results: results_url: {results_url}')

    # Check results_url
    if (results_url == None):
        return None

    # Save results to database
    results['id'] = experiment_id
    results['name'] = experiment_name
    results_df = DataFrame(
        results,
        index=None,
        columns=['id', 'name', 'roc_auc', 'accuracy', 'precision', 'recall']
    )
    df_to_sql(results_df, results_url, table_name)

    return None

def save_model(model, X, experiment_id, experiment_name, table_name=None, logger:Logger=None):
    ''' Saves the model to the registry. If the environment variable
        MODEL_URL is set, then save_model inserts the path to the model
        within the registry to the database. '''
    
    # Get registry uri
    remote_registry = os.getenv('REGISTRY_URI')

    # Log remote_registry
    if (logger):
        logger.debug(f'save_model: remote_registry: {remote_registry}')

    # Save model to remote registry
    mlflow.set_tracking_uri(remote_registry)
    mlflow.set_experiment(experiment_name)
    with mlflow.start_run():
        # Log the model hyperparameters
        mlflow.log_params(model.get_params())

        # Infer the model signature
        signature = infer_signature(X, model.predict(X))

        # Log the model
        model_info = mlflow.sklearn.log_model(
            sk_model=model,
            signature=signature,
            input_example=X
        )
    
    # Get models url
    models_url = os.getenv('MODELS_URL')

    # Log models_url
    if (logger):
        logger.debug(f'save_model: model_url: {models_url}')

    # Check models_url
    if (models_url == None):
        return None

    # Save model id to models database
    df_to_sql(
        DataFrame(
            data={
                'id': [experiment_id],
                'name': [experiment_name],
                'model_id': [model_info.model_id]
            },
            index=None,
            columns=['id', 'name', 'model_id']
        ),
        models_url,
        table_name
    )

    return None
    
def run(config, logger):
    ''' Run the experiment. '''
    logger.info(f'Runinng experiment')
    logger.debug(config)

    # Create experiement id
    experiment_id = create_experiment_id(config, logger)
    logger.debug(f'experiment_id: {experiment_id}')

    # Load training data
    logger.info(f'Loading training data')
    X, y = load_data(config['train_path'], logger)
    logger.debug(f'X.shape: {X.shape}')
    logger.debug(f'y.shape: {y.shape}')

    # Load test data
    logger.info(f'Loading test data')
    X_test, y_test = load_data(config['test_path'], logger)
    logger.debug(f'X_test.shape: {X_test.shape}')
    logger.debug(f'y_test.shape: {y_test.shape}')

    # Get model
    logger.info(f'Getting model')
    model = get_model(config['model'], logger)

    # Fit model
    logger.info(f'Fitting model')
    model.fit(X,y)

    # Evaluating model
    logger.info(f'Evaluating model')
    results = evaluate(model, X_test, y_test)
    logger.debug(f'results: {results}')

    # Save results to results database
    logger.info('Saving results to database')
    save_results(results, experiment_id, config['name'], config['results_table'], logger)

    # Save the model to the model registry
    logger.info('Saving model to registry')
    save_model(model, X_test, experiment_id, config['name'], config['models_table'], logger)

# Run experiment
if __name__ == '__main__':
    # Configure logger
    import logging.config
    with open(os.getenv('LOGGER_CONFIG', default='logger.yaml'), 'rb') as f:
        config = yaml.safe_load(f)
    logging.config.dictConfig(config)

    # Create logger
    logger = logging.getLogger(__name__)
    logger.setLevel(os.getenv('LOG_LEVEL', default='INFO'))

    # Load experiment config
    with open(os.getenv('EXPERIMENT_CONFIG', default='../configs/experiment.yaml'), 'rb') as f:
        config = yaml.safe_load(f)

    # Run experiment
    run(config, logger)
