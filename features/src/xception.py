...
if not os.path.exists(os.path.join(PROCESSED_DATA_DIR, 'xception')):
    os.mkdir(os.path.join(PROCESSED_DATA_DIR, 'xception'))

with open(os.path.join(PROCESSED_DATA_DIR,"xception/train_data.csv"), "w", newline="") as csvfile:
...
with open(os.path.join(PROCESSED_DATA_DIR,"xception/test_data.csv"), "w", newline="") as csvfile:
    writer = csv.writer(csvfile)
    col_names = ["x{}".format(i) for i in range(model.output.shape[-1])]
    col_names.append("y")
    writer.writerow(col_names)
